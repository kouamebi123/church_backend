...
// Récupérer les détails d'un réseau pour l'inscription publique des responsables de GR
exports.getPublicNetworkDetails = async (req, res) => {
  try {
    const { prisma } = req;
    const { id } = req.params;
    ...
    // Récupérer les groupes avec leurs responsables
    const groups = await prisma.group.findMany({
      where: { network_id: id },
      select: {
        id: true,
    ...
    logger.error('Network - getPublicNetworkDetails - Erreur', error);
    const { status, message } = handleError(error, 'la récupération des détails du réseau');
    res.status(status).json({
      success: false,
      message
    });
